# The config9 baseline

The library example as this configuration's rules derive it into an empty VSUM, with no
migration involved. Every cell in the row `config9-to-*` starts from this state, and every
cell in the column `*-to-config9` is read against it: a migration that ends where this
does has re-derived everything the target rules would have produced anyway.

Rules: `umljava-config9.jar`, built from `../rules/config9-reactions`.
