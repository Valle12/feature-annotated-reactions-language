# The config5 baseline

The library example as this configuration's rules derive it into an empty VSUM, with no
migration involved. Every cell in the row `config5-to-*` starts from this state, and every
cell in the column `*-to-config5` is read against it: a migration that ends where this
does has re-derived everything the target rules would have produced anyway.

Rules: `umljava-config5.jar`, built from `../rules/config5-reactions`.
