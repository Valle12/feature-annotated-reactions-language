# The config2 baseline

The library example as this configuration's rules derive it into an empty VSUM, with no
migration involved. Every cell in the row `config2-to-*` starts from this state, and every
cell in the column `*-to-config2` is read against it: a migration that ends where this
does has re-derived everything the target rules would have produced anyway.

Rules: `umljava-config2.jar`, built from `../rules/config2-reactions`.
