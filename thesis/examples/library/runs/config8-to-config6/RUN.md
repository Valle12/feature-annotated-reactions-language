...
| items left to deal with by hand | 15 |
...
The migrated models describe exactly what `baselines/config6` describes. Under `--preserve report` nothing is carried over into the models, so what is left is what the target rules derive - and that is what the baseline derives from the same UML.
