...
| dirty rules | 19 (0 added, 19 removed) |
...
The migrated models and `baselines/config5` do not describe the same thing about 8 classifier(s), which is the number the matrix carries. Each line below names a classifier and how the two differ, per model, so a classifier wrong in both is listed twice here and counted once there; `expected` is the baseline.
